...
func AccountDeactivateSignIn(t *testing.T, a func(*testing.T) authgo.Authenticator) {
...
func AccountDeactivateSignUp(t *testing.T, a func(*testing.T) authgo.Authenticator) {
	tmpl, err := template.New("sign-up.go.html").Parse(`{{.Error}}`)
	assert.Nil(t, err)
	auth := a(t)
	authtest.NewTestAccount(t, auth)
	token, _ := authtest.SignIn(t, auth)
	...
	reader := strings.NewReader("email=" + authtest.TEST_EMAIL + "&username=" + authtest.TEST_USERNAME + "&password=" + authtest.TEST_PASSWORD + "&confirmation=" + authtest.TEST_PASSWORD)
	request = httptest.NewRequest(http.MethodPost, "/sign-up", reader)
	request.Header.Set("Content-Type", "application/x-www-form-urlencoded")
	response = httptest.NewRecorder()
	mux.ServeHTTP(response, request)
	...
	body, err := io.ReadAll(result.Body)
	assert.Nil(t, err)
	assert.Equal(t, authgo.ErrEmailAlreadyRegistered.Error(), string(body))
}
