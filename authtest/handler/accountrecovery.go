...
func AccountRecovery(t *testing.T, a func() authgo.Authenticator) {
	...
	t.Run("Redirects When Email Is Registered", func(t *testing.T) {
		...
		assert.Nil(t, auth.AccountManager().SetEmailVerified(acc.Email, true))
		mux := http.NewServeMux()
		handler.AttachHandlers(auth, mux, tmpl)
		request := httptest.NewRequest(http.MethodGet, "/account-recovery", nil)
		response := httptest.NewRecorder()
...
func AccountRecoveryVerification(t *testing.T, a func() authgo.Authenticator) {
	...
	t.Run("Returns 200 When Recovering", func(t *testing.T) {
		auth := a()
		authtest.NewTestAccount(t, auth)
		token, err := auth.SessionManager().NewAccountRecovery()
		assert.Nil(t, err)
		mux := http.NewServeMux()
		handler.AttachHandlers(auth, mux, tmpl)
